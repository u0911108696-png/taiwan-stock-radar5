...
useEffect(() => {
  ...
  return top50
    .map((stock) => {
      ...
      if (isNextDay) {
        score += 15;
        reasons.push("列入明日優先觀察");
      }

      if (stock.changePercent > 8) {
        score -= 25;
        reasons.push("短線漲太多，防追高");
      }

      if (stock.price < stock.openPrice) {
        score -= 20;
        reasons.push("跌破開盤價，轉弱");
      }
    ...
    <div className="grid grid-cols-2 gap-2">
      ...
      <div className="rounded-2xl bg-black/40 px-3 py-2 text-right">
        ...
        <div className="text-2xl font-black text-cyan-200">
          {yesterdayHighWinCompareList.filter((item) => item.continued).length}
        </div>
      </div>
    </div>
  </div>
